...
in_port_t TCPCommunicator::getPortInfo(struct sockaddr *sa){
...
void * TCPCommunicator::getIPAddress(struct sockaddr *sa){
  if (sa->sa_family == AF_INET) {
...
TCPCommunicator* TCPCommunicator::acceptConnection(){
  ...
  client->socket.ip = string((char *)getIPAddress(&clientsockaddr));
